...
from src.core import python_errors
...
class CommandCreateCommandsList:
    ...
    def is_returned(self) -> bool:
        for com in self.__commands:
            if isinstance(com, CommandReturn):
                return True
        return False
...
class CommandFunCreate:
    def __init__(self, memory: memory.RunTimeMemory, name, commands, returned, file_name, arguments) -> None:
        self.__memory = memory
        self.__name = name
        self.__expression = commands
    ...
    def exec(self):
        self.__memory.WriteObject(
            memory.MemoryObject(
                self.__name, 
                luma_values.FunctionValue.Create(
                ...
                False, memory.MemoryObjectTypes.FUNCTION, False
            )
        )
...
class CommandIf:
    def __init__(self, condition_expression, commands_true, commands_else = None) -> None:
        self.__condition = condition_expression
        self.__commands_true = commands_true
        self.__commands_else = commands_else

    def exec(self):
        if self.__condition.eval().value == True:
            self.__commands_true.exec()
        elif self.__commands_else != None:
            self.__commands_else.exec()
...
class CommandReturn:
    ...
    def exec(self):
        raise luma_exceptions.Return(self.__expression)

class CommandTryCatch:
    def __init__(self, command, error_command, lang, python_error = None) -> None:
        self.__command = command
        self.__error_command = error_command
        self.__lang = lang
        self.__python_error = python_error

    def exec(self):
        if self.__lang == 'py':
            if self.__python_error is not None and self.__python_error in python_errors.python_error:
                try:
                    self.__command.exec()
                except python_errors.python_error[self.__python_error] as err:
                    self.__error_command.exec()
            else:
                try:
                    self.__command.exec()
                except:
