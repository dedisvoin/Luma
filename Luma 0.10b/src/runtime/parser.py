...
class Parser:
    ...
    def parse_commands(self):
        ...
        if self.is_token(tokens.TokenSignature.KEYVORD_IF):
            return self.construct_command_If(command_start_pos)
        if self.is_token(tokens.TokenSignature.BRACKET_LEFT_CURLY):
            return self.parse_commands_list_for_CommandsList()
        if self.is_token(tokens.TokenSignature.KEYWORD_TRY):
            return self.construct_command_TryCatch(command_start_pos)
        if self.is_token(tokens.TokenSignature.KEYWORD_RETURN):
            return self.construct_command_Return(command_start_pos)
        if self.is_token(tokens.TokenSignature.KEYWORD_FUN):
            return self.construct_command_FunCreate(command_start_pos)
            
        return self.parse_expression()
    
    def construct_command_FunCreate(self, command_start_pos):
        self.match(tokens.TokenSignature.KEYWORD_FUN)
        self.is_token_and_wait(tokens.TokenSignature.WORD, 'function name')
        fun_name  =self.get_token_value()
        self.match(tokens.TokenSignature.WORD)
        wait_argument_names = self.parse_waited_arguments()
        returned = False
        if self.is_token(tokens.TokenSignature.OPERATOR_STRELA_RIGHT): returned = True
        self.match(tokens.TokenSignature.OPERATOR_STRELA_RIGHT)
        code_block = self.parse_commands_list_for_CommandsList()
        return commands.CommandFunCreate(self.__run_time_memory, fun_name, code_block, returned, self.__file_name, wait_argument_names)
    
    def construct_command_Return(self, command_start_pos):
        self.match(tokens.TokenSignature.KEYWORD_RETURN)
        expr = self.parse_expression()
        self.is_token_and_wait(tokens.TokenSignature.OPERATOR_DOT_AND_COMMA, ';')
        return commands.CommandReturn(expr)
    
    def construct_command_TryCatch(self, command_start_pos):
        self.match(tokens.TokenSignature.KEYWORD_TRY)
        self.match_and_wait(tokens.TokenSignature.BRACKET_LEFT_STANDART, '(')
        self.is_token_and_wait(tokens.TokenSignature.WORD, 'error to language (py, lm)')
        lang = self.get_token_value()
        self.match(tokens.TokenSignature.WORD)
        self.match_and_wait(tokens.TokenSignature.BRACKET_RIGHT_STANDART, ')')
        comm = self.parse_commands()
        self.match_and_wait(tokens.TokenSignature.KEYWORD_CATCH, 'catch')
        err = None
        if self.is_token(tokens.TokenSignature.STRING):
            err = self.get_token_value()
            self.match(tokens.TokenSignature.STRING)
        err_comm = self.parse_commands()
        return commands.CommandTryCatch(comm, err_comm, lang, err)

    def construct_command_If(self, command_start_pos):
        self.match(tokens.TokenSignature.KEYVORD_IF)
        self.match_and_wait(tokens.TokenSignature.BRACKET_LEFT_STANDART, '(')
        condition = self.parse_expression()
        self.match_and_wait(tokens.TokenSignature.BRACKET_RIGHT_STANDART, ')')
        command_true = self.parse_commands()
        command_false = None
        
        if self.is_token(tokens.TokenSignature.KEYVORD_ELSE):
            self.match_and_wait(tokens.TokenSignature.KEYVORD_ELSE, 'else')
            command_false = self.parse_commands()
        return commands.CommandIf(condition, command_true, command_false)
    ...
    def construct_command_For(self, command_start_pos):
        ...
        self.match_and_wait(tokens.TokenSignature.OPERATOR_DOT_AND_COMMA, ';')
        self.match_and_wait(tokens.TokenSignature.BRACKET_RIGHT_STANDART, ')')
        executed_commands = self.parse_commands()
        self.is_token_and_wait(tokens.TokenSignature.OPERATOR_DOT_AND_COMMA, ';')
    ...
    def parse_math_binare_multi(self):
        ...
        while True:
            ...
            if self.match(tokens.TokenSignature.OPERATOR_MUL):
                expression = expressions.ExpressionMathBinary('*', self.__file_name, expression, self.parse_math_unary(), parse_pos_start, self.get_token_pos())
                continue
            if self.match(tokens.TokenSignature.OPERATOR_PER):
                expression = expressions.ExpressionMathBinary('%', self.__file_name, expression, self.parse_math_unary(), parse_pos_start, self.get_token_pos())
                continue
            break
        return expression
    ...
    def parse_basic(self):
        ...
        if self.is_token(tokens.TokenSignature.KEYWORD_FUN):
            ...
            self.match(tokens.TokenSignature.OPERATOR_STRELA_RIGHT)
            code_block = self.parse_commands_list_for_CommandsList()
            return expressions.ExpressionFunObject(wait_argument_names, code_block, self.__file_name, returned)
