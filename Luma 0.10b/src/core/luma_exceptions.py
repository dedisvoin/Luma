...
class LumaExceptionTypes:
    ...
    CALL = f'{Fore.BLACK}call{Fore.RESET}'
...
class ErrorFunNotReturn(BaseLumaException):
    def __init__(self, file: str | None = None, pos: tokens.TokenPosition | None = None, name: str = '') -> None:
        super().__init__(LumaExceptionTypes.CALL, file, pos)
        self.name = name
        self.exp_type = COMPILER

    def LumaRaise(self):
        self.raise_()
        print(f"|   In position {str(self.pos)}. Function {Fore.YELLOW}'{self.name}'{Fore.RESET} not returned.")
        self.stop_run_time()